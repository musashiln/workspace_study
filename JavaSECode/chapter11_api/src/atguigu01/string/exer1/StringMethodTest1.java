...
//        System.out.println(s4.isEmpty());//报空指针异常
     ...
     * (15)int lastIndexOf(String str, int fromIndex):返回指定子字符串在此字符串中最后一次出现处的
     */
    @Test
    public void test3(){
        String s1="教育尚硅谷教育";
     ...
     * (25)boolean startsWith(String prefix, int toffset):测试此字符串从指定索引开始的子字符串是否
    ...
    /**
     * (27) String replace(char oldChar, char newChar):返回一个新的字符串,它是通过用newChar替换此字符串中出
     * 现的所有oldChar得到的。不支持正则。
     * (28) String replace(CharSequence target,CharSequence replacement):使用指定的字面值替换序列替换此字符串
     * 所有匹配字面值目标序列的子字符串。
     * (29) String replaceAll(String regex,String replacement):使用给定的replacement 替换此字符串所有匹配给定的
     * 正则表达式的子字符串。
     * (30) String replaceFirst(String regex,String replacement):使用给定的replacement 替换此字符串匹配给定的正
     * 则表达式的第一个子字符串。
     */
    @Test
    public void test6(){
